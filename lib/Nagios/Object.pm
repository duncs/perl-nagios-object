...
sub attribute_allows_undef {
    my $type = ref($_[0]) ? ref($_[0]) : $_[0];
    return 1 if ( ${"$type\::valid_fields"}->{$_[1]}[2] != 0 );
    undef;
}
...
sub _validate {
    my( $self, $key, $value, $type, $multi_f, $undef_f, $possible ) = @_;
    ...
    if ( $type eq 'BINARY' ) {
        ...
        # automatically split , delmited strings
        ...
        croak "$key cannot have multiple values for ".ref($value)
            if ( ref($value) eq 'ARRAY' && $multi_f == 0 );

        # put single objects into an ARREF so code is consistent
        ...
        # return single object if $multi_f is turned off, otherwise, return the list
        if ( $multi_f == 0 ) { $value = $value->[0] }
    }
    else {
...
BEGIN {
    ...
    #   Object => {
    #      attribute => [ Type, Multiple Flag, Undef Allowed Flag, Nagios ],
    #   }
    ...
    # Multiple Flag: specify whether or not the field can contain an array
    #                reference, which must then contain elements all of the
    #                proper type at validation time
    ...
    my %_nagios_setup = (
        Service => {
            use                           => ['Nagios::Service',      0, 0, 1],
            service_description           => ['STRING',               0, 0, 1],
			host_name                     => ['Nagios::Host',         1, 0, 1],
			hostgroup_name                => ['Nagios::HostGroup',    1, 0, 1],
			is_volatile                   => ['BINARY',               0, 0, 1],
			check_command                 => ['Nagios::Command',      0, 0, 1],
			max_check_attempts            => ['INTEGER',              0, 0, 1],
			normal_check_interval         => ['INTEGER',              0, 0, 1],
			retry_check_interval          => ['INTEGER',              0, 0, 1],
			active_checks_enabled         => ['BINARY',               0, 0, 1],
			passive_checks_enabled        => ['BINARY',               0, 0, 1],
			check_period                  => ['Nagios::TimePeriod',   0, 1, 1],
			parallelize_check             => ['BINARY',               0, 0, 1],
			obsess_over_service           => ['BINARY',               0, 0, 1],
			check_freshness               => ['BINARY',               0, 0, 1],
			freshness_threshhold          => ['INTEGER',              0, 0, 1],
			event_handler                 => ['Nagios::Command',      0, 0, 1],
			event_handler_enabled         => ['BINARY',               0, 0, 1],
			low_flap_threshhold           => ['INTEGER',              0, 0, 1],
			high_flap_threshhold          => ['INTEGER',              0, 0, 1],
			flap_detection_enabled        => ['BINARY',               0, 0, 1],
			process_perf_data             => ['BINARY',               0, 0, 1],
			retain_status_information     => ['BINARY',               0, 0, 1],
			retain_nonstatus_information  => ['BINARY',               0, 0, 1],
			notification_period           => ['Nagios::TimePeriod',   0, 1, 1],
			notification_interval         => ['Nagios::TimePeriod',   0, 1, 1],
            notification_options          => ['LIST',         1, 0, 'uwcr', 1],
            contact_groups                => ['Nagios::ContactGroup', 1, 0, 1],
            notifications_enabled         => ['BINARY',               0, 0, 1],
			stalking_options              => ['LIST',         1, 0, 'owuc', 1],
            name                          => ['service_description',  0, 0, 0],
        ...
        Host => {
		    ...
		    parents                       => ['Nagios::Host',         1, 1, 1],
		    check_command                 => ['STRING',               0, 0, 1],
		    max_check_attempts            => ['INTEGER',              0, 0, 1],
		    checks_enabled                => ['BINARY',               0, 0, 1],
		    event_handler                 => ['STRING',               0, 0, 1],
		    event_handler_enabled         => ['BINARY',               0, 0, 1],
		    low_flap_threshhold           => ['INTEGER',              0, 0, 1],
		    high_flap_threshhold          => ['INTEGER',              0, 0, 1],
		    flap_detection_enabled        => ['BINARY',               0, 0, 1],
		    ...
		    retain_status_information     => ['BINARY',               0, 0, 1],
		    retain_nonstatus_information  => ['BINARY',               0, 0, 1],
		    notification_period           => ['Nagios::TimePeriod',   0, 0, 1],
		    notification_interval         => ['Nagios::TimePeriod',   0, 0, 1],
		    notification_options          => ['LIST',          1, 0, 'dur', 1],
		    notifications_enabled         => ['BINARY',               0, 0, 1],
		    stalking_options              => ['LIST',          1, 0, 'odu', 1],
            name                          => ['host_name',            0, 0, 0],
            comment                       => ['comment',              0, 0, 0],
            file                          => ['filename',             0, 0, 0]
        },
        HostGroup => {
            hostgroup_name                => ['STRING',               0, 0, 1],
	        alias                         => ['STRING',               0, 0, 1],
	        contact_groups                => ['Nagios::ContactGroup', 1, 0, 1],
	        members	                      => ['Nagios::Host',         1, 0, 1],
            name                          => ['hostgroup_name',       0, 0, 0],
            comment                       => ['comment',              0, 0, 0],
            file                          => ['filename',             0, 0, 0]
        },
        Contact => {
			...
			service_notification_period   => ['Nagios::TimePeriod',   0, 0, 1],
	        host_notification_options     => ['LIST',        1, 0, 'durn',  1],
			service_notification_options  => ['LIST',        1, 0, 'wucrn', 1],
			host_notification_commands    => ['Nagios::Command',      1, 0, 1],
			service_notification_commands => ['Nagios::Command',      1, 0, 1],
			email                         => ['STRING',               0, 0, 1],
			pager                         => ['STRING',               0, 0, 1],
            name                          => ['contact_name',         0, 0, 0],
            comment                       => ['comment',              0, 0, 0],
            file                          => ['filename',             0, 0, 0]
        },
        ContactGroup => {
            contactgroup_name             => ['STRING',               0, 0, 1],
	        alias                         => ['STRING',               0, 0, 1],
	        members	                      => ['Nagios::Contact',      1, 0, 1],
            name                          => ['contactgroup_name',    0, 0, 0],
            comment                       => ['comment',              0, 0, 0],
            file                          => ['filename',             0, 0, 0]
        },
        Command => {
            command_name                  => ['STRING',               0, 0, 1],
            command_line                  => ['STRING',               0, 0, 1],
            name                          => ['command_name',         0, 0, 0],
            comment                       => ['comment',              0, 0, 0],
            file                          => ['filename',             0, 0, 0]
        },
        TimePeriod => {
            ...
            monday                        => ['TIMERANGE',            0, 0, 1],
            tuesday                       => ['TIMERANGE',            0, 0, 1],
            wednesday                     => ['TIMERANGE',            0, 0, 1],
            thursday                      => ['TIMERANGE',            0, 0, 1],
            friday                        => ['TIMERANGE',            0, 0, 1],
            saturday                      => ['TIMERANGE',            0, 0, 1],
            name                          => ['timeperiod_name',      0, 0, 0],
            comment                       => ['comment',              0, 0, 0],
            file                          => ['filename',             0, 0, 0]
        }, 
        ServiceEscalation => {
			host_name                     => ['Nagios::Host',         0, 0, 1],
        ...
        ServiceDependency => {
            dependent_host                => ['Nagios::Host',         0, 0, 1],
	        dependent_service             => ['Nagios::Service',      0, 0, 1],
			host_name                     => ['Nagios::Host',         0, 0, 1],
			service                       => ['Nagios::Service',      0, 0, 1],
    ...
    GENESIS: {
        ...
        foreach my $object ( keys(%_nagios_setup) ) {
            ...
            # create a method to return the method name to call that should
            # return the textual name of an object
            #if ( my $name = delete($_nagios_setup{$object}->{name}) ) {
            #    *{"$pkg\::_name_attribute"} = sub { $name };
            #}

            # create the "type" method
            my $pkg_type = lc($object);

            # create methods for each entry in $_nagios_setup{$object}
            foreach my $method ( keys(%{$_nagios_setup{$object}}) ) {

                if ( $_nagios_setup{$object}->{$method}[-1] == 0 ) {
                    my $value = delete( $_nagios_setup{$object}->{$method} );
                    *{"$pkg\::_${method}_attribute"} = sub { $value->[0] };
                    next;
                }
